...
DropButton {
    ...
    function fitMapViewportToAllCoordinates(coordList) {
        ...
        // Determine if we need to fit the width or height of the bounding region with the viewport
        var fitHeight = true
        var fitViewportWidthToHeightRatio = mapFitViewport.width / mapFitViewport.height
        var regionWidthToHeightRatio = (east - west) / (north - south)
        if (regionWidthToHeightRatio < fitViewportWidthToHeightRatio) {
            fitHeight = true
        } else {
            fitHeight = false
        }
        ...
        if (fitHeight) {
            ...
            var boundingHeight = north - south
            var delta = ((boundingHeight / fitViewportWidthToHeightRatio) - boundingHeight) / 2
            south -= delta
            north += delta
        }
        ...
        mapRectangle.topLeft = topLeftCoord
        mapRectangle.bottomRight = bottomRightCoord
        map.visibleRegion = QtPositioning.rectangle(topLeftCoord, bottomRightCoord)
    }
    ...
    dropDownComponent: Component {
        ColumnLayout {
            ...
            QGCButton {
                text:               qsTr("Vehicle")
                Layout.fillWidth:   true
                enabled:            _activeVehicle && _activeVehicle.latitude != 0 && _activeVehicle.longitude != 0
